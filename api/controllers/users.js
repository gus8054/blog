...
export async function registerController(req, res) {
  ...
  // 데이터 존재 확인 검증
  if (!(username.trim() && password.trim() && email.trim())) return res.status(400).json({ message: "data is required." });
  // 아이디 중복 확인
  ...
  db.query(selectQuery, [username, email], (err, results) => {
    ...
    if (results.length) return res.status(400).json({ message: "중복된 아이디입니다." });
    // 패스워드 암호화
...
export const checkUserController = (req, res) => {
  const username = req.params.username;
