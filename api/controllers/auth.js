...
export async function loginController(req, res) {
  ...
  // 데이터 존재 확인 검증
  if (!(username.trim() && password.trim())) return res.status(400).json({ message: "data is required" });
  // 아이디 존재 확인
  const selectQuery = "SELECT * FROM users WHERE username = ?";
  db.query(selectQuery, [username], (err, results) => {
    if (err) return res.status(500).json({ message: "서버 오류" }); // 서버 오류
    if (!results.length) return res.status(404).json({ message: "user not exist" });
    const foundUser = results[0];
    // 찾은 계정과 비밀번호 일치 확인
    bcrypt.compare(password, foundUser.password, async function (err, isEqual) {
      if (err) return res.status(500).json({ message: "서버 오류" }); // 서버 오류
      if (!isEqual) return res.status(401).json({ message: "invalid password" }); // 비밀번호 틀림
      // jwt payload에 담을 객체 생성
      const { password, ...others } = foundUser;
      // jwt 객체 생성 - access token
      const accessToken = jwt.sign(others, process.env.ACCESS_TOKEN_SECRET, { issuer: "DH", expiresIn: "15s" });
      // jwt 객체 생성 - refresh token
      const refreshToken = jwt.sign(others, process.env.REFRESH_TOKEN_SECRET, { issuer: "DH", expiresIn: "1d" });
      ...
      const selectRefreshQuery = "SELECT * FROM refreshtokens WHERE username = ?";
      db.query(selectRefreshQuery, [foundUser.username], (err, results) => {
        if (err) return res.status(500).json({ message: "서버 오류" }); // 서버 오류

        const query = results.length ? "UPDATE refreshtokens SET refreshtoken = ? WHERE (username = ?)" : "INSERT INTO refreshtokens (refreshtoken, username) VALUES (?, ?)";
...
export async function refreshtokenController(req, res) {
  ...
  if (!refreshToken) return res.status(403).json({ message: "refresh token not exist" });
  // refresh token 검증
  jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, function (err, decoded) {
    ...
    db.query(selectQuery, [username], (err, results) => {
      if (err) return res.status(500).json({ message: "서버 오류" }); // 서버 오류
      if (!results.length) return res.status(403).json({ message: "Forbidden" });
      const accessToken = jwt.sign({ username, role, email }, process.env.ACCESS_TOKEN_SECRET, { issuer: "DH", expiresIn: "15s" });
      return res.status(200).json({ accessToken });
    });
  });
}
