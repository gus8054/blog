...
export const addPost = async (req, res) => {
  ...
  // 데이터 존재 확인 검증
  if (!(category.trim() && sanitizedContent.trim() && imageURL.trim() && rowContent.trim() && title.trim())) return res.status(400).json({ message: "data is required." });
  const id = uuidv4();
  const q = "INSERT INTO posts (id, title, sanitized_content, row_content, image_url, category, author) VALUES (?)";
  const value = [id, title, sanitizedContent, rowContent, imageURL, category, username];
...
export const getPosts = async (req, res) => {
  ...
  q += count ? ` ORDER BY created_at DESC LIMIT ${count}` : ` ORDER BY created_at DESC LIMIT 5`;
